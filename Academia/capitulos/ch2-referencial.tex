...
O resultado obtido é de certo modo inesperado, já que conforme demonstrados nas subseções \ref{sec-dijkstra-versoes-heap} e \ref{sec-dijkstra-versoes-fibonacci}, o heap de Fibonacci possui tempo computacional, aplicado ao algoritmo de Dijkstra, é $O(\log |V|)$ enquanto o heap binário possui $O(|E| \log |V|)$. Por consequência, a dedução lógica do ponto de vista teórico é que heap de Fibonacci apresentasse tempos mais rápidos do que o Heap Binário.
